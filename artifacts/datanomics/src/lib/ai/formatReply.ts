/** Ensure recruiter replies use professional paragraph breaks. */
export function formatProfessionalReply(body: string, channel?: string): string {
  let text = body.replace(/\r\n/g, '\n').trim();
  if (!text) return text;

  // Unescape literal \n from JSON if model returned them as two-char sequences
  text = text.replace(/\\n/g, '\n');

  if (channel === 'linkedin' || channel === 'phone') {
    // Shorter channels: single blank lines between logical blocks
    return text.replace(/\n{3,}/g, '\n\n').trim();
  }
